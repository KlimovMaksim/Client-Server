...
{
    public partial class UpdateRemoveDestination : Form
    {
        public UpdateRemoveDestination()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            using (DataTable dt = new DataTable())
            {
                using (SqlConnection conn = new SqlConnection(Data.value))
                {
                    try
                    {
                        using (SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM Destination ORDER BY DestinationId ASC", conn))
                        {
                            da.Fill(dt);
                        }
                    }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            using (SqlConnection conn = new SqlConnection(Data.value))
            {
                using (SqlCommand cmd = new SqlCommand("EXEC UpdateDestination '" + textBox1.Text + "', '" + textBox2.Text + "', '" + textBox3.Text + "', '" + textBox4.Text + "', '" + textBox5.Text + "'", conn))
                {
                    conn.Open();
                    try
                    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            using (SqlConnection conn = new SqlConnection(Data.value))
            {
                using (SqlCommand cmd = new SqlCommand("EXEC RemoveDestination '" + textBox1.Text + "'", conn))
                {
                    conn.Open();
                    try
                    {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                {
                    textBox1.Text = current.Cells["DestinationId"].Value.ToString();
